...
class ProductIndex extends Component
{
    ...
    public function resetForm()
    {
        ...
        $this->editStockOriginal = 0;
        $this->editStockReason = '';
        $this->resetErrorBag();
    }
    ...
    public function edit($id)
    {
        ...
        $this->editStockOriginal = (int) $product->stock_quantity;
        $this->editStockReason = '';
    ...
    public function save($keepOpen = false)
    {
        ...
        // Sửa tồn kho trong form: GIẢM tồn -> bắt buộc nhập lý do (tăng thì áp dụng ngay).
        if ($this->productId) {
            $existing = Product::find($this->productId);
            if ($existing) {
                $oldStock = (int) $existing->stock_quantity;
                $newStock = (int) ($this->stock_quantity === '' || $this->stock_quantity === null ? 999 : $this->stock_quantity);
                if ($newStock < $oldStock && trim((string) $this->editStockReason) === '') {
                    $this->addError('editStockReason', 'Giảm tồn kho cần nhập lý do.');
                    return;
                }
            }
        }
        ...
        if ($this->productId) {
            ...
            if ($newStock !== $oldStock) {
                $note = $newStock < $oldStock
                    ? 'Sửa SP (giảm tồn): ' . trim((string) $this->editStockReason)
                    : 'Điều chỉnh khi sửa sản phẩm (tăng tồn)';
                $product->recordStockHistory(
                    'Adjustment', $newStock - $oldStock,
                    null, null, $note, $oldStock
                );
            }
