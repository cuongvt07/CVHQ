...
class ProductIndex extends Component
{
    ...
    public function updatedSku($value): void
    {
        ...
        if (preg_match('/\d+$/', $value)) {
            // The user entered/edited a specific numbered SKU, so do not auto-rewrite it.
            $this->isAutoGenerated = false;
            return;
        }
        ...
        $nextSku = $this->nextSkuForPrefix($value);
        if ($nextSku !== $value) {
            $this->sku = $nextSku;
     ...
     * Case-insensitive matching, dynamically adapting to existing SKU digits padding.
     */
    private function nextSkuForPrefix(string $prefix, ?int $padding = null): string
    {
        ...
        $maxNumber = 0;
        $maxPadding = 0; // Initialize to 0 to dynamically adapt to existing digit padding
        $rx = '/^' . preg_quote($prefix, '/') . '(\d+)$/i'; // Case-insensitive matching
        $hasExisting = false;
