<div class="h-full flex flex-col">
    ...
    <header class="px-4 md:px-6 py-3 border-b border-slate-200 bg-slate-50/50 flex items-center justify-between gap-2 flex-wrap">
        ...
        <div class="flex items-end gap-2 flex-wrap">
            ...
            <a href="{{ route('reports.commission_staff.print', ['month' => $month, 'branch' => $branch]) }}" target="_blank" rel="noopener"
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden max-w-3xl mx-auto">
                ...
                <table class="w-full text-left text-[13px] whitespace-nowrap">
                    <thead class="bg-emerald-50 border-b border-emerald-100">
                        <tr>
                            ...
                            <th class="px-3 py-2.5 text-[10px] font-bold text-slate-600 uppercase">Nhân viên</th>
                            <th class="px-3 py-2.5 text-[10px] font-bold text-slate-600 uppercase text-right">Số đơn</th>
                            <th class="px-3 py-2.5 text-[10px] font-bold text-slate-600 uppercase text-right">Doanh số</th>
                            <th class="px-3 py-2.5 text-[10px] font-bold text-slate-600 uppercase text-right">Hoa hồng</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                        @forelse($rows as $i => $r)
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-3 py-2 text-right text-slate-600">{{ $fmt($r['cnt']) }}</td>
                                <td class="px-3 py-2 text-right text-slate-700">{{ $fmt($r['sales']) }}</td>
                                <td class="px-3 py-2 text-right font-black text-electric-blue">{{ $fmt($r['commission']) }}</td>
                            </tr>
                        @empty
                            <tr><td colspan="5" class="px-3 py-10 text-center text-slate-400">Chưa có nhân viên nhận hoa hồng.</td></tr>
                        @endforelse
                    </tbody>
                    <tfoot class="bg-emerald-50/60 border-t-2 border-emerald-200">
                        <tr>
                            ...
                            <td class="px-3 py-2.5 text-right font-black text-slate-800">{{ $fmt($totals['cnt']) }}</td>
                            <td class="px-3 py-2.5 text-right font-black text-slate-800">{{ $fmt($totals['sales']) }}</td>
                            <td class="px-3 py-2.5 text-right font-black text-electric-blue">{{ $fmt($totals['commission']) }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
