<div class="h-full flex flex-col">
    <header class="px-4 md:px-6 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 bg-slate-50/50">
        <div>
            <h1 class="text-lg md:text-xl font-black tracking-tight text-slate-900 uppercase">Quản lý khách hàng</h1>
        </div>
    ...
    @php $__activeFilterCount = 0; /* search stays inline; perPage and column-toggle are not counted as filters */ @endphp
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100 bg-white/50">
                    @foreach($customers as $customer)
                        <tr wire:key="customer-row-{{ $customer->id }}" class="hover:bg-slate-50 transition-colors group/row {{ in_array((string)$customer->id, $selectedRows) ? 'bg-electric-blue/5' : '' }}">
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <button type="button" 
                                        ...
                                        class="relative inline-flex h-5 w-10 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none {{ $customer->status === 'Active' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]' : 'bg-slate-200' }}">
