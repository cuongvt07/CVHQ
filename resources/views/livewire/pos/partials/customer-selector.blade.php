    ...
    @if($selectedCustomer)
        <div class="flex items-center justify-between bg-electric-blue/5 border border-electric-blue/10 rounded-lg px-2 py-1.5">
            <div class="flex items-center gap-2">
                ...
                <div>
                    <p class="text-xs font-bold text-slate-900 leading-tight">{{ $selectedCustomer->full_name }}</p>
                    <p class="text-[9px] text-slate-400">{{ $selectedCustomer->phone }}</p>
                </div>
            </div>
            <button wire:click="clearCustomer" class="text-slate-300 hover:text-red-500 transition-colors">
        ...
        <div class="relative" x-data="{ open: @entangle('show_customer_search') }">
            <div class="flex gap-2">
                <div class="relative flex-1">
                    ...
                    <input type="text" wire:model.live.debounce.300ms="customer_search" @focus="open = true" placeholder="Tìm khách hàng..." class="w-full bg-white border border-slate-200 rounded-lg pl-9 pr-3 py-1.5 text-[11px] focus:outline-none focus:border-electric-blue transition-all text-slate-900">
                </div>
                <button wire:click="$set('is_creating_customer', true)" class="w-8 h-8 rounded-lg bg-white border border-slate-200 flex items-center justify-center text-slate-400 hover:text-electric-blue hover:border-electric-blue/50 transition-all" title="Tạo khách hàng mới">
