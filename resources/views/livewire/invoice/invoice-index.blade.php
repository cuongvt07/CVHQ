    ...
    <div class="px-4 md:px-6 py-4 bg-white border-b border-slate-100 flex flex-col gap-4">
        ...
        @if($startDate || $endDate || $sellerFilter || $search || $channelFilter)
            <div class="flex flex-wrap items-center gap-2 animate-in fade-in slide-in-from-top-1 duration-200">
                ...
                @if($channelFilter)
                    @php $activeChannel = $sales_channels->firstWhere('id', (int) $channelFilter); @endphp
                    <div class="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-bold text-white group shadow-sm"
                         style="background-color: {{ $activeChannel?->color ?? '#64748b' }};">
                        <span class="opacity-80">Kênh:</span> {{ $activeChannel?->name ?? '—' }}
